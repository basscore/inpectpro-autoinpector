...
export default function ChecklistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Pencarian item lintas-kategori. Saat ada query, tampilkan item yang cocok dari
  // semua kategori (bukan hanya tab aktif) agar inspektor cepat menemukannya.
  const normalizedQuery = searchQuery.trim().toLowerCase();
  const isSearching = normalizedQuery.length > 0;
  const searchResults: { item: any; categoryName: string }[] = isSearching
    ? categories.flatMap((cat) =>
        cat.items
          ...
          .map((it: any) => ({ item: it, categoryName: cat.name }))
      )
    : [];
  ...
  return (
    <div className="min-h-screen bg-surface-secondary flex flex-col pb-24">
      ...
      <div className="sticky top-0 z-30 bg-primary-dark text-white shadow-md">
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2 bg-white/10 rounded-xl text-sm text-white placeholder:text-white/50 outline-none focus:bg-white/15 focus:ring-2 focus:ring-white/20 transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-lg text-white/60 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
        ...
        {/* Category Tabs */}
        <div className={`flex overflow-x-auto gap-1 px-4 pb-3 scrollbar-none ${isSearching ? "opacity-40 pointer-events-none" : ""}`}>
        ...
        {isSearching ? (
          <>
            <p className="text-[11px] text-text-tertiary px-1">
              {searchResults.length} item cocok dengan &quot;{searchQuery.trim()}&quot;
            </p>
            {searchResults.length === 0 ? (
              <div className="bg-white rounded-2xl border border-dashed border-border p-8 text-center animate-fade-in">
          ...
          <>
            ...
            {currentCategory?.items.map((item: any) => renderItemCard(item))}
